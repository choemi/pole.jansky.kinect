...
void draw() {
  ...
  // blur the blob image
  // HIER KANN MAN verschieden filter anwenden auf die silhouette von der person, z.b. parameter von blur erhöhen auf zwei,
  //dann wirkt es nicht mehr so pixelig, wird aber dann auch dünner wenn jemand weiter weg ist...
  blobs.filter(BLUR, 1);
  // detect the blobs
...
void updateAndDrawBox2D() {
  ...
  /* MIT FRAMERATE kann man jetzt anzahl die anzahl SVG grafiken erhöhen bzw. reduzieren die erscheinen. allerdings ist dass
  wahrscheinlich nur ein effekt wegen der rechenpower oder anzeigeleistung... weil SVG grafiken eine höhere framerate haben
  */
  if (frameRate > 10) {
    ...
    /* KOMSCHES VERHALTEN, wenn shape1 und 2 gleichzeit drinn sind, und die shape1 von hier unten nicht, dann werden SVG
    diese anderen objekte angezeigt. Wenn nur die shape1 von oben drinn ist, werden nur die objekte angezeigt. Wenn nur shape1
    von unten drinn ist, werden nur die SVG angezeigt...
    */
    
    CustomShape shape1 = new CustomShape(random(50, kinectWidth-50), random(50, kinectHeight-50), 40, BodyType.DYNAMIC);
    polygons.add(shape1);
    //polygons.add(shape2);
  }
