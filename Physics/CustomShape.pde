...
class CustomShape {
  ...
  void makeBody(float x, float y, BodyType type) 
  {
    ...
    if (r == -1) 
    {
      /* HIER IRGENDWOE MUSS dieses SVG file reingeladen werden entweder als PolygonShape oder toxiPoly */
  ...
  void display() {
    ...
    if (r == -1) {
      ...
      // IST NICHT WIRKLICH KORREKT, weil alles andere immer noch gemacht wird, und es sollte nur noch das SVG geladen werden
